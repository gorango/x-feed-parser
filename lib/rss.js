...
/** @typedef {import('../index.js').PaginationLinks} PaginationLinks */
 ...
 * @returns {Feed} feed
 */
export function buildRSSFeed(xmlObj, options) {
	let feed
	if (xmlObj.rss && xmlObj.rss.$ && xmlObj.rss.$.version && xmlObj.rss.$.version.match(/^2/)) {
		feed = buildRSS2(xmlObj, options)
	}
	else if (xmlObj['rdf:RDF']) {
		feed = buildRSS1(xmlObj, options)
	}
	else if (xmlObj.rss && xmlObj.rss.$ && xmlObj.rss.$.version && xmlObj.rss.$.version.match(/0\.9/)) {
		feed = buildRSS0_9(xmlObj, options)
	}
	else if (xmlObj.rss && options.defaultRSS) {
 ...
 * @returns {Feed} feed
 */
function buildRSS0_9(xmlObj, options) {
 ...
 * @returns {Feed} feed
 */
function buildRSS1(xmlObj, options) {
 ...
 * @returns {Feed} feed
 */
function buildRSS2(xmlObj, options) {
	const channel = xmlObj.rss.channel[0]
	const items = channel.item
