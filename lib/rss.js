...
export function buildRSSFeed(xmlObj) {
	...
	const fn = [buildRSS2, buildRSS1, buildRSS0]
	const fnIdx = [isRSS2, isRSS1, isRSS0].indexOf(true)
	return fn[fnIdx](xmlObj)
}

function buildRSS0(xmlObj) {
...
function buildRSS(channel, items = []) {
	...
	const feed = {}
	feed.type = 'rss'
	feed.title = decodeXML(specFeed.title)
...
function parseItemRss(xmlItem) {
	...
	if (specItem.lang)
		item.lang = specItem.lang

	if (specItem.title)
		item.title = decodeXML(specItem.title)

	if (specItem.summary)
		item.commentsUrl = specItem.summary

	if (specItem.author)
		item.author = specItem.author
	...
	if (xmlItem['media:group']?.[0]?.['media:description'])
		item.content = getContent(xmlItem['media:group']?.[0]?.['media:description'])
