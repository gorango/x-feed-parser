...
function parseItemAtom(xmlItem) {
	...
	if (xmlItem.title) {
		...
		if (title._)
			title = title._.trim()
		if (title)
			item.title = decodeXML(title).trim()
	}

	if (xmlItem.summary && xmlItem.summary.length)
		item.summary = getContent(xmlItem.summary[0])?.trim()
	...
	if (xmlItem.content && xmlItem.content.length) {
		item.content = getContent(xmlItem.content[0])?.trim()
		item.snippet = getSnippet(item.content)?.trim()
	}

	if (!item.content && xmlItem['media:group']?.[0]?.['media:description']?.[0]) {
		item.content = decodeXML(xmlItem['media:group']?.[0]?.['media:description'][0]).trim()
		item.snippet = getSnippet(item.content).trim()
	}
