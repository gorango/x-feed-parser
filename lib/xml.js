...
import { copyFromXml, getCategories, getContent, getImageUrl, getMeta, parseXml } from './utils.js'
 ...
 * @returns {Feed} feed
 */
export function buildXmlFeed(str) {
	...
	feed.items = (items || [])?.map(item => parseItemRss(item, lang))
 ...
 * @returns {Item} item
 */
function parseItemRss(xmlItem, lang = 'en') {
	if (!xmlItem)
		return null
	...
	if (xmlItem?.['media:group']) {
		const group = xmlItem['media:group']
		if (group['media:description'])
			item.content = decodeXML(group['media:description'])?.trim()
